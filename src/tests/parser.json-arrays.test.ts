...
test('Returns type for json array of primatives', async () => {
    ...
    expect((await parser.dump()).replace(tabNewline, '')).toBe("export type test = Array<string>;");

    parser.load('[1]', "test");

    expect((await parser.dump()).replace(tabNewline, '')).toBe("export type test = Array<number>;");

    parser.load('[true]', "test");

    expect((await parser.dump()).replace(tabNewline, '')).toBe("export type test = Array<boolean>");
});

test('Returns typed array for nested array', async () => {
    const parser = new Parser('[["a"]]', "test");

    expect((await parser.dump()).replace(tabNewline, '')).toBe("export type test = Array<Array<string>>");
});

test('Returns any array for nested array', async () => {
    const parser = new Parser('[["a"], [1]]', "test");

    expect((await parser.dump()).replace(tabNewline, '')).toBe("export type test = Array<Array<any>>");
});